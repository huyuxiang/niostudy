...
public class Server {
	...
	public synchronized void start() {
		if (!isStart) {
			...
			factory = new NioServerSocketChannelFactory(Executors.newFixedThreadPool(2), Executors.newFixedThreadPool(9), 8);
	...
	public void execute(int seed) {
		while (isStart) {
			...
			try {
				...
				Object result = methodSupport.invoke(parameters.getInterfaceName(), parameters.getParameterTypes().toArray(clazzArray), parameters.getParameters().toArray());
