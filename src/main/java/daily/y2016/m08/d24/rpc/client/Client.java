...
import daily.y2016.m08.d24.rpc.network.handler.ResultHandler;
...
public class Client {
	...
	public synchronized void start() {
		if(isStart) {
			throw new RuntimeException ("client is start");
		} else {
			isStart = true;
			
			channelFactory = new NioClientSocketChannelFactory(
					Executors.newFixedThreadPool(10),
					Executors.newFixedThreadPool(10), 1, 8);
	...
	public synchronized void stop () {
		if(!isStart) {
			throw new RuntimeException("client stop");
		} else {
			isStart = false;
		}
	}
